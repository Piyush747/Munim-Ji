...
class cashin():
    ...
    def __init__(self,root):
        self.root = root
        self.root.title("Cash In - Munim Ji")
        ...
        tk.Label(self.root,text="Enter your earnings here...",font=("Helvetica",12,BOLD),fg="white",bg="#FF4C29",height=2).pack(fill=X)
        ...
        add_btn = tk.Button(self.root,command=self.add_cashin,text="Add",font=("Helvetica",10,BOLD),bg="lime green",fg="white",cursor="hand2").place(x=10,y=290,width=120,height=30)
        cancel_btn = tk.Button(self.root,command=self.cancel_cashin,text="Cancel",font=("Helvetica",10,BOLD),bg="tomato",fg="white",cursor="hand2").place(x=140,y=290,width=120,height=30)
    ...
    def add_cashin(self):
        ...
        try:
            if(self.title_txt.get()=="" or self.amount_txt.get()==""):
                messagebox.showerror("Cash In - Munim Ji","Title and Amount are mandatory fields, cannot be left empty.",parent=self.root)
                self.root.focus_force()
            elif(self.amount_txt.get().isnumeric()==False):
                messagebox.showerror("Cash In - Munim Ji","Amount says - Invalid Input.",parent=self.root)
                self.root.focus_force()
            else:
                ...
                con.commit()
                con.close()
                messagebox.showinfo("Cash In Successful - Munim Ji","The record has been added sucessfully.",parent=self.root)
                self.root.destroy()
        except Exception as e:
            print(e)
            messagebox.showerror("Error","Oops some errror occured. Please try again later.",parent=self.root)

    def cancel_cashin(self):
        self.root.destroy()
